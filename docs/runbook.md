...
Deploy не запускает seed автоматически. Для demo-данных или первого admin-пользователя используйте отдельную команду `make seed` после настройки `SEED_ADMIN_EMAIL` и `SEED_ADMIN_PASSWORD`.

Production guard включается через `APP_ENV=production`. Локальный Docker по умолчанию использует `APP_ENV=development`, потому что `next start` сам выставляет `NODE_ENV=production`.
...
powershell -ExecutionPolicy Bypass -File scripts/project.ps1 seed
...
Infra-порты в compose привязаны к `127.0.0.1`, чтобы Postgres/Redis/MinIO не открывались наружу на сервере. Публичными остаются `web` и `api`.
...
Доступ разрешён только пользователям с ролью `admin`. Seed создаёт администратора только если заданы `SEED_ADMIN_EMAIL` и `SEED_ADMIN_PASSWORD`.
