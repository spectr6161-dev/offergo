...
export const paymentStatuses = ["pending", "confirmed", "canceled", "chargebacked", "expired"] as const;
...
export const plategaStatusSchema = z.enum(["PENDING", "CONFIRMED", "CANCELED", "CHARGEBACKED"]);
...
const envSchema = z.object({
  ...
  DATABASE_URL: z.string().min(1).default("postgresql://offergo:offergo@localhost:5434/offergo_app?schema=public"),
  REDIS_URL: z.string().min(1).default("redis://localhost:6379"),
  BETTER_AUTH_URL: z.string().url().default("http://localhost:3000"),
  BETTER_AUTH_SECRET: z.string().min(16).default("offergo-dev-secret-please-change"),
  ...
  PLATEGA_SUCCESS_URL: z.string().url().default("http://localhost:3000/billing?status=success"),
  PLATEGA_FAIL_URL: z.string().url().default("http://localhost:3000/billing?status=fail"),
