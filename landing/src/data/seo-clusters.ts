...
export const seoClusterPages: SeoClusterPage[] = [
  ...
  },
  {
    ...
    title: "Проверка резюме IT — помощь с резюме программиста",
    description:
      "Проверка резюме IT-специалиста: резюме программиста, junior frontend/backend/mobile, AI-анализ, улучшение формулировок и подготовка к откликам.",
    h1: "Проверьте резюме разработчика перед откликами",
    eyebrow: "Резюме и оффер",
    lead:
      "Резюме разработчика должно быстро объяснять стек, опыт, роль в проектах и результат. offerGO помогает найти слабые места до отправки отклика.",
    primaryKeywords: [
      "проверка резюме IT",
